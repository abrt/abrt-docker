...
RUN sed 's/\(journalctl \)--system/\1 -J \/host\/var\/log\/journal /' -i /etc/libreport/events.d/ccpp_event.conf
RUN sed 's/\(journalctl.*-b\)/\1 -J \/host\/var\/log\/journal /' -i /etc/libreport/events.d/ccpp_event.conf
